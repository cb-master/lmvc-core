...
class Zip
{
    ...
    ## --------------------------- PUBLIC API --------------------------- ##
    ########################################################################
    // Initiate ZIP Instance
    public function __construct(string $path)
    {
        $dir = dirname($path);

        // Check Valid File if File Exists
        if(file_exists($path) && !is_file($path)) throw new InvalidArgumentException("Path '{$path}' is not a valid file!");

        if(!is_dir($dir) || !is_writable($dir)) throw new InvalidArgumentException("Directory '{$dir}' is not writable!");
    ...
    /**
     * Create ZIP Archive
     * @param string|array $files Directory or list of files to include in the archive.
     */
